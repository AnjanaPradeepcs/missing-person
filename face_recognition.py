...
def scan_known_people(known_people_folder):
 ...
 if len(encodings) > 1:
 click.echo("WARNING: More than one face found in {}. Only considering the first 
face.".format(file))
 if len(encodings) == 0:
...
def dostuff(video_capture, known_face_names, known_face_encodings, ct, mobcam = False, count = 
0):
 face_locations = []
 face_encodings = []
 face_names = []
...
tolerance=tolerance)
 ...
 cv2.imwrite('./finds/'+ '_'.join(name.split())+'-'+'_'.join(str(lastseen.lastseendict[name][-
1][0]).split())+'.jpg', frame )
 if ct == 0:
...
def main(ct):
 ...
 mobcam = True
 if mobcam:
 address = 'http://100.126.162.54:8080/video'
 else:
 address = 0
